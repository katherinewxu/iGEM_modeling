...
transition_probabilities = {
    "S": np.array([0.0, 1, 0.000000, 0.00000, 0.000000]),  
    "E": np.array([0.33, 0.00000, 0.667, 0.00000, 0.000000]),  
    "I": np.array([0.000000, 0.00000, 0.0, 0.041, 0.959]), 
    "R": np.array([0.000000, 0.00000, 0.832, 0.00000, 0.168])
}

# Remove transitions with zero probabilities
filtered_transition_probabilities = {}
for state, probabilities in transition_probabilities.items():
    non_zero_indices = probabilities > 0
    filtered_transition_probabilities[state] = probabilities[non_zero_indices]

# Print the transition probabilities
for state, probabilities in filtered_transition_probabilities.items():
    ...
    for _ in range(time_steps):
        ...
        for state, count in state_distribution.items():
            if count > 0 and state in transition_probabilities:
                ...
                # Find the possible states for this particular state's transitions
                possible_states = [states[i] for i in range(len(probs))]
                
                transitions = np.random.multinomial(count, probs)
                for i, next_state in enumerate(possible_states):
...
print(adjusted_transition_probabilities)

# Use adjusted_transition_probabilities in the bayesian_optimization function
