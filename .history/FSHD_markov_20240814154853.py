...
adjusted_transition_probabilities = {
    state: np.insert(probs / num_steps, 0, 1 - np.sum(probs / num_steps))
    if np.sum(probs) > 0 else np.array([1.0])
...
simulation_history = simulate_markov_model(adjusted_transition_probabilities, initial_state_distribution, 3)
        ...
        for state, count in state_distribution.items():
            if count > 0 and state in transition_probabilities:
                ...
                for i, next_state in enumerate(states):
                    new_distribution[next_state] += np.random.multinomial(count, probs)[i]
            else:
                new_distribution[state] += count
        state_distribution = new_distribution.copy()
        history.append(state_distribution.copy())
