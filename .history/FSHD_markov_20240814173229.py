...
for state, probabilities in transition_probabilities_daily.items():
    ...
    if sum(non_zero_indices) > 0:  # Only adjust non-terminal states
        ...
        transition_probabilities_hourly[state] = np.array([1.0])  # Terminal state stays the same
# Convert daily transition probabilities to hourly with a stay probabilitytransition_probabilities_hourly = {}for state, probabilities in transition_probabilities_daily.items():    non_zero_indices = probabilities > 0    transition_probabilities_hourly[state] = np.zeros_like(probabilities)    if sum(non_zero_indices) > 0:  # Only adjust non-terminal states        transition_probabilities_hourly[state][non_zero_indices] = 1 - np.power(1 - probabilities[non_zero_indices], 1/24.0)        stay_prob = 1 - np.sum(transition_probabilities_hourly[state])        transition_probabilities_hourly[state][0] = stay_prob  # Assign the stay probability to the first index (or another appropriate position)    else:        transition_probabilities_hourly[state] = np.array([1.0])  # Terminal state stays the same
# Convert daily transition probabilities to hourly with a stay probability
transition_probabilities_hourly = {}
for state, probabilities in transition_probabilities_daily.items():
    non_zero_indices = probabilities > 0
